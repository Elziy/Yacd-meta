...
import { fetchProxies, getDelay, getProxies } from '~/store/proxies';
...
function ProxyImpl({
  ...
  const testLatency = React.useCallback(async () => {
  ...
  return (
    ...
    >
      ...
      <div className={s0.row}>
        ...
        <ProxyLatency isLoading={isTestingLatency} handleOnclick={testLatency} number={latencyNumber} color={color} />
      </div>
    </div>
  );
}
